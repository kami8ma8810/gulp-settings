...
jQuery(function () {
  ...
  var hRatio = 0.8,
      //画面のどこまで要素が来た時表示させるか。:初期値0.8(画面上より80%の位置)
  fTime = 400,
      //fuwatto1発動までに必要な時間:初期値400(0.4秒)
  ...
  if ($fuwattoWrap.length) {
    //fuwatto-wrapにクラス付与
    $fuwattoWrap.addClass("not-fuwatto"); //ふわっと処理実行

    actFuwatto(); //スクロール時の処理
  ...
  function actFuwatto() {
    ...
    $(".not-fuwatto").each(function () {
      var $this = $(this),
          thisPos = $this.offset().top; //$thisの位置が画面の80%の位置に来た場合のみ処理

      if (thisPos <= wHeight * hRatio + nowScroll) {
        //$thisからnot-fuwattoクラスを消去
        $this.removeClass("not-fuwatto"); //fTime秒後に実行
        ...
        setTimeout(function () {
          ...
          var kaz = $this.find("[class*='fuwatto']").length,
              finish = 0,
              fuwattoTimer,
              fIndex = 2; //$thisの子要素にfuwatto1が存在する場合のみ処理
